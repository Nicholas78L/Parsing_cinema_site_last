...
for b in range(0, len(links_genres)):
    with open('film_databaseTest_3.1/cinema_'+links_genres[b]+'_.csv', 'w') as template_file:
...
# будут доступны только "свежие" данные о фильмах.
with open('film_databaseTest_3.1/cinema_parsing_all_in_one.csv', 'w') as general_file:
...
for items in dataAll:
    with open('film_databaseTest_3.1/cinema_parsing_all_in_one.csv', 'a') as general_file:
        writer = csv.writer(general_file)
        writer.writerow(
            items
        )
...
for g in range(0, len(links_genres)):                       # Цикл перебора названий жанров из их кол-ва (32)
    ...
    for f in range(0, len(dataAll)):                        # Цикл перебора каждого фильма из общей базы данных
        if genres[g] in dataAll[f][2] and count < stop:     # Проверяем наличие названия жанра (кириллицей) в общей базе данных
            with open('film_databaseTest_3.1/cinema_' + links_genres[g] + '_.csv', 'a') as final_file:      # заполняем csv файлы
